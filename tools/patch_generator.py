...
class PatchGenerator(object):
    def __init__(self, info, type='normal', data_dir='./data/PANDA/image_train',
                 save_img_path='./data/PANDA/patch', save_json_path='./new.json'):
    ...
    def data_transfer(self):
        for num, json_file in enumerate(self.info):
            ...
            if self.type == 'center':
                ...
                center_list = self.slide_window(width, height, [(2048 * 4, 1024 * 4)], [(2048 * 4, 1024 * 4)])
            for patch_id, center_lf_point in enumerate(center_list):
                ...
                for obj in self.info[json_file]['objects list']:
                    if obj['category'] not in ['full body', 'vehicles']:
                        continue
                    self.supercategory = obj['category']
                    if self.supercategory not in self.label:
                        self.categories.append(self.categorie())
                        self.label.append(self.supercategory)
                    x1 = float(obj['rect']['tl']['x'] * width)
                    y1 = float(obj['rect']['tl']['y'] * height)
                    w = float((obj['rect']['br']['x'] - obj['rect']['tl']['x']) * width)
                    h = float((obj['rect']['br']['y'] - obj['rect']['tl']['y']) * height)
...
def worker1(train_info):
    PatchGenerator(train_info, type='sw',
                   save_json_path='./data/PANDA/train_lin_full_v.json')


def worker2(val_info):
    PatchGenerator(val_info, type='sw', save_json_path='./data/PANDA/val_lin_full_v.json')


if __name__ == '__main__':
    file = open('./person_vehicle.json', 'r', encoding='utf-8')
    info = json.load(file)
    ...
    p2 = multiprocessing.Process(target=worker2, args=(val_info,))
